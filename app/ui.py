...
#         if st.button(
#             ":arrows_counterclockwise: SEARCH"
#         ):
#             similar_image = Image.open(io.BytesIO(segments.content)).convert("RGB")
...
if input_image is not None:
    ...
    if st.button(":arrows_counterclockwise: SEARCH"):
        ...
        Path = data["similar_images"][0]
        similar_image = Image.open(Path).convert("RGB")
        with col2:
